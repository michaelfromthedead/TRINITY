...
import { PREFIX, SEPARATOR } from '../adapters/constants'
import { LitegraphLinkAdapter } from '../adapters/linkAdapter'
import type { LinkRenderContext } from '../adapters/linkAdapter'
import { getSlotPosition } from '../adapters/slotCalculations'
import { useLayoutMutations, layoutStore, LayoutSource, removeNodeTitleHeight } from '../adapters/layout'
import { forEachNode } from '../adapters/graphTraversal'
...
export class LGraphCanvas implements CustomEventDispatcher<LGraphCanvasEventMap> {
  ...
  onNodeDblClicked?: (n: LGraphNode, pos?: [number, number], e?: MouseEvent) => void
  ...
  #processNodeClick(
    ...
    if (widget) {
      ...
      pointer.onDoubleClick = () => {
        ...
        this.processNodeDblClicked(node, pos, e)
      }
  ...
  processNodeDblClicked(n: LGraphNode, pos?: [number, number], e?: MouseEvent): void {
    this.onShowNodePanel?.(n)
    this.onNodeDblClicked?.(n, pos, e)
  ...
  computeVisibleNodes(nodes?: LGraphNode[], out?: LGraphNode[]): LGraphNode[] {
    ...
    for (const node of _nodes) {
      // Skip nodes with hidden flag (for type filtering)
