/**
 * Keybinding Store - Keyboard shortcut management for FlowForge
...
// Reserved key combinations that shouldn't be captured in text inputs
const RESERVED_BY_TEXT_INPUT = new Set([
  'Ctrl + A',
  'Ctrl + C',
  'Ctrl + V',
  'Ctrl + X',
...
/**
 * Parses a key combo string like 'ctrl+shift+s' into a KeyCombo object.
 */
function parseKeyCombo(combo: string): KeyCombo | null {
  ...
  for (const part of parts) {
    ...
    switch (trimmed) {
      ...
      default:
        ...
        keyCombo.key = trimmed.length === 1 ? trimmed.toUpperCase() : trimmed
        break
    }
  }

  if (!keyCombo.key) return null
...
export const useKeybindingStore = defineStore('keybinding', () => {
  ...
  /**
   * Register default keybindings from a simple format.
   * @param bindings Array of { combo: string, command: string } where combo is like 'ctrl+s'
   */
  function registerDefaultKeybindings(
    bindings: { combo: string; command: string }[]
  ) {
    for (const binding of bindings) {
      const keyCombo = parseKeyCombo(binding.combo)
      if (keyCombo) {
        try {
          ...
          console.warn(`Failed to register keybinding ${binding.combo}:`, error)
        }
      }
    }
  }
  ...
  /**
   * Register core file operation keybindings.
   */
  function registerCoreKeybindings() {
    // NOTE: Keybindings are registered by App.vue's registerKeybindings()
    // which maps to App.vue's command IDs (File.Save, Edit.Undo, etc.).
    // Do NOT register keybindings here as they would conflict and block
    // App.vue's registrations (addDefaultKeybinding throws on duplicates).
    // The FlowForge.* commands from registerCoreFileCommands() are available
    // via command palette but not via keybindings (App.vue's commands handle those).
  }
