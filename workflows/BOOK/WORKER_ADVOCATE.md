...
During SCRIBE_LOOP, some source documents contradict each other on a manuscript concept — an authorial decision, a structural choice, a pedagogical framing, a definition, a chapter ordering claim — and neither temporal supersession nor explicit replacement resolves which is correct. SCRIBE flags the concept as a CONFLICT in MASTER.
...
- **The concept under dispute** (its name, what it concerns — e.g., "chapter ordering: does Chapter 3 or Chapter 4 introduce spin?")
- **Your side**: A or B
...
- Fabricate evidence (citations must be real — quotations must be verbatim from actual source documents)
...
**Part B: Evidence.** The strongest citations for your side. Quote source docs exactly. Reference section or approximate location. Include:

- Direct statements in source docs supporting your value
- Manuscript coherence — does your value fit the rest of the manuscript structure better? (e.g., if your side proposes introducing spin before angular momentum, does the rest of MASTER assume spin is already established by chapter 3?)
- Temporal reasoning — is your value from a more-refined / later draft?
- Load-bearing — does your value underlie more other manuscript decisions? (e.g., a pedagogical sequencing decision that many later chapters assume)
- Authorial intent — does your value align better with explicit authorial statements about the book's structure or pedagogical contract?
...
**Weak:** "Side A is supported by the later drafts."

**Strong:** "Side A is supported by `chapter_3_draft_v4.md:§2` which explicitly states 'I've decided to reverse the ordering — spin comes before angular momentum — because readers need the concrete case first.' This directly supersedes `outline_v2.md:§3.2` which proposed the reverse."

Cite files and quoted excerpts. Vague appeals to "later" or "more refined" without quotes weaken the brief.

### 5.2 Engage the opposition

**Weak:** Present only your side's case.

**Strong:** "Side B's strongest argument is likely that `early_outline.md` (the document that first established the chapter structure) placed spin after angular momentum, suggesting that was the author's original design intent. However, `revision_notes_2026-03.md` explicitly abandons that design citing reader confusion in test readings — this directly undermines the original-intent argument."
...
**Strong:** Three or more distinct reasons, each from a different angle (temporal, manuscript coherence, authorial intent, load-bearing-ness).
...
QUEEN is going to apply decision criteria (see `BOOK_CONSOLIDATION.json` §`court_mechanism.decision_criteria_ordered`). Help QUEEN by acknowledging which criteria favor your side vs. the opposition.
...
- **Temporal primacy:** <...>
- **Evidentiary weight:** <...>
- **Manuscript coherence:** <does my value fit the rest of MASTER's content and structure better?>
- **Load-bearing-ness:** <does my value underlie more downstream manuscript decisions?>
...
- Citations are real (every source referenced exists and contains the claimed content): yes
...
| Treating manuscript conflicts as less important than engineering conflicts | Authorial framing, pedagogical sequencing, and conceptual definitions ARE real conflicts with real consequences for the book's coherence |
...
- **SYNTHESIS** — QUEEN interjects a combined value drawing from both sides (e.g., a merged treatment of the disputed concept)
- **DEFER** — QUEEN kicks the issue to COMPOSITOR or to human
