...
def calculate_doppler_shift(
    ...
    if isinstance(source_pos, tuple):
        source_pos = Vec3(*source_pos)
    if isinstance(source_velocity, tuple):
        source_velocity = Vec3(*source_velocity)
    if isinstance(listener_pos, tuple):
        listener_pos = Vec3(*listener_pos)
    if isinstance(listener_velocity, tuple):
        listener_velocity = Vec3(*listener_velocity)
    ...
    # Note: direction points FROM source TO listener
    # Source moving toward listener = moving WITH direction = positive dot = negated for approaching
    # Listener moving toward source = moving AGAINST direction = negative dot = negated for approaching
    source_approach = -source_velocity.dot(direction)
...
class DopplerProcessor:
    ...
    def process(
        ...
        """Process position/velocity data and return pitch shift.

        Args:
            source_pos: Source position (Vec3 or tuple).
            source_velocity: Source velocity (Vec3 or tuple).
            listener_pos: Listener position (Vec3 or tuple).
            listener_velocity: Listener velocity (Vec3 or tuple).

        Returns:
            Pitch shift multiplier.
        """
        self._last_shift = calculate_doppler_shift(
            source_pos=source_pos,
            source_velocity=source_velocity,
            listener_pos=listener_pos,
            listener_velocity=listener_velocity,
            doppler_factor=self._doppler_factor,
