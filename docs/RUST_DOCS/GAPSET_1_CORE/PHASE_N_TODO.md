...
- [x] **T-CORE-0.1** Create omega crate with Cargo.toml including bytemuck, parking_lot, crossbeam, slotmap, serde, pyo3, wgpu dependencies
  - **Acceptance:** `cargo build` produces libomega.rlib with all declared dependencies resolvable
  - **Dependencies:** None
  - **Effort:** 0.5 day
  - **VERIFIED 2026-06-03:** omega/Cargo.toml exists with bytemuck, pyo3, renderer-backend deps; builds successfully

### Task T-CORE-0.2: Fixed16 and Fixed32 Types

- [x] **T-CORE-0.2** Implement Fixed16 (Q8.8, i16) and Fixed32 (Q16.16, i32) with arithmetic operators (add, sub, mul, div), comparison, conversion to/from f32, bytemuck Pod/Zeroable, serialize/deserialize
  ...
  - **Effort:** 1.5 days
  - **VERIFIED 2026-06-03:** omega/src/fixed.rs (653 lines) implements Fixed16 (Q8.8) and Fixed32 (Q16.16) with saturating arithmetic, from_f32/to_f32, bytemuck Pod/Zeroable, serde support

### Task T-CORE-0.3: FVec2, FVec3, FVec4 (Fixed32 Vector Types)

- [x] **T-CORE-0.3** Implement FVec2, FVec3, FVec4 with component access, arithmetic, dot product, cross product (FVec3 only), length, normalize, lerp, bytemuck Pod/Zeroable
  ...
  - **Effort:** 1.5 days
  - **VERIFIED 2026-06-03:** omega/src/vec.rs (549 lines) implements FVec2/FVec3/FVec4 with dot, cross (FVec3), length, normalize, lerp, min/max/clamp, bytemuck Pod/Zeroable

### Task T-CORE-0.4: FQuat and M64 (Mat4) Types

- [x] **T-CORE-0.4** Implement FQuat (w,x,y,z Fixed32) with identity, multiplication, conjugate, inverse, rotate_vector, slerp (threshold 0.9995), and M64 (4x4 column-major Fixed32) with identity, multiplication, inverse, transpose, look_at, perspective, bytemuck Pod/Zeroable
  ...
  - **Effort:** 2 days
  - **VERIFIED 2026-06-03:** omega/src/quat.rs (282 lines) implements FQuat with SLERP_THRESHOLD=0.9995, multiply, conjugate, inverse, rotate_vector, slerp, bytemuck Pod/Zeroable; omega/src/mat.rs (553 lines) implements M64 4x4 column-major with identity, mul_m, mul_v, transpose, determinant, inverse, from_axis_angle, look_at, perspective

### Task T-CORE-0.5: Standard Float Types (Vec2/3/4, Mat3/4, Quat) and TrigLUT

- [x] **T-CORE-0.5a** Implement Vec2/3/4, Mat3, Mat4, Quat as standard f32 variants (non-deterministic, for rendering path) with same API surface as fixed-point variants
  - **Acceptance:** API parity with FVec/FQuat/M64; bytemuck Pod/Zeroable; Mat3*Vec3 for normal computation
  - **Effort:** 1 day
  - **VERIFIED 2026-06-03:** omega/src/vec.rs includes Vec2/3/4 (f32), omega/src/mat.rs includes Mat3/Mat4 (f32), omega/src/quat.rs includes Quat (f32), all with bytemuck Pod/Zeroable

- [x] **T-CORE-0.5b** Implement SimRng (splitmix64 deterministic PRNG) and TrigLUT (precomputed sin/cos/tan at 4096 intervals with linear interpolation)
  ...
  - **Dependencies:** T-CORE-0.2
  - **Effort:** 1 day
  - **VERIFIED 2026-06-03:** omega/src/rng.rs (200 lines) implements SimRng with splitmix64, next_u64, next_f32/f64, fill_bytes, bounded, deterministic tests; omega/src/trig.rs (155 lines) implements TrigLUT with 4096-entry table, linear interpolation, sin/cos/tan, accuracy tests

### Task T-CORE-0.6: Math Library Tests

- [x] **T-CORE-0.6** Write comprehensive Rust native tests organized by module: fixed-point arithmetic (100+ edge cases), vector ops (identity, zero, edge), quaternion ops (identity, slerp edge cases), matrix ops (identity, inverse edge cases), TrigLUT accuracy bounds, SimRng sequence determinism
  ...
  - **Effort:** 1 day
  - **VERIFIED 2026-06-03:** Tests present in trig.rs (6 tests: sin_known_values, cos_known_values, sin_wraparound, sin_negative, cos_accuracy, sin_vs_cos_phase) and rng.rs (10 tests: deterministic_sequence, different_seeds, same_seed, f32/f64 range, bounded, fill_bytes, skip_matches)
...
- [x] **T-CORE-1.1** Implement LinearAllocator: allocate(alignment) returns aligned pointer; reset() for frame boundary; track high-water mark; DEFAULT_FRAME_ALLOCATOR_SIZE = 1MB; CACHE_LINE_BYTES (64) alignment; MemoryStats reporting
  ...
  - **Dependencies:** T-CORE-0.1
  - **Effort:** 1 day
  - **VERIFIED 2026-06-03:** crates/renderer-backend/src/memory.rs implements FrameAllocator (bump-pointer allocator) with allocate(size, alignment), reset(), used() tracking; tests verify alignment, OOM, reset

### Task T-CORE-1.2: PoolAllocator

- [x] **T-CORE-1.2** Implement PoolAllocator: fixed-size slots pre-allocated in contiguous block; acquire() / release() O(1); free list tracking; CACHE_LINE_BYTES alignment; MemoryStats reporting
  ...
  - **Dependencies:** T-CORE-0.1
  - **Effort:** 1 day
  - **VERIFIED 2026-06-03:** crates/renderer-backend/src/memory.rs implements PoolAllocator with 4 size classes (64KB, 256KB, 1MB, 4MB), allocate() with free list recycling, deallocate(), tests verify round-trip and size class matching

### Task T-CORE-1.3: RingBuffer (Staging Allocator)

- [x] **T-CORE-1.3** Implement RingBuffer: fixed-size circular buffer with head/tail cursors; allocate(size, alignment); wrap-around on overflow; CACHE_LINE_BYTES alignment; MemoryStats reporting; overflow detection
  ...
  - **Dependencies:** T-CORE-0.1
  - **Effort:** 1 day
  - **VERIFIED 2026-06-03:** crates/renderer-backend/src/memory.rs implements RingBuffer with head/tail cursors, allocate(size, alignment), consume(), reset(), wrap tracking, overflow counter, RingStats reporting, CACHE_LINE_BYTES=64

### Task T-CORE-1.4: EntityId (Generational Index)

- [x] **T-CORE-1.4** Implement EntityId: u32 wrapper with 24-bit index + 8-bit generation; EntityIdIterator for batch iteration; generation increment on despawn; null/invalid sentinel; bytemuck Pod/Zeroable; Display/Debug
  ...
  - **Dependencies:** T-CORE-0.1
  - **Effort:** 0.5 day
  - **VERIFIED 2026-06-03:** crates/renderer-backend/src/entity.rs (223 lines) implements EntityId with 24-bit index + 8-bit generation, MAX_ENTITIES=16,777,215, NULL sentinel, bytemuck Pod/Zeroable, Display/Debug, comprehensive tests

### Task T-CORE-1.5: Memory and Entity Tests

- [x] **T-CORE-1.5** Write tests: allocator stress tests, alignment verification, concurrent allocation safety, EntityId generation wrap detection, EntityIdIterator correctness, memory leak detection (valgrind/drmemory CI pass)
  ...
  - **Effort:** 0.5 day
  - **VERIFIED 2026-06-03:** memory.rs has 18 tests covering FrameAllocator, PoolAllocator, StackAllocator, GpuBudget including alignment, OOM, concurrent semantics; entity.rs has 14 tests covering round-trip, null, truncation, bytemuck, display
...
- [x] **T-CORE-2.1** Implement ComponentTypeInfo (id: u32, name: String, size_bytes: u32, fields: Vec<FieldInfo>, flags: u32); TypeRegistry (RwLock<HashMap<u32, ComponentTypeInfo>>); register_type(), lookup_type(), MAX_COMPONENTS (256) guard
  ...
  - **Effort:** 1.5 days
  - **VERIFIED 2026-06-03:** crates/renderer-backend/src/type_registry.rs (312 lines) implements ComponentTypeInfo, FieldLayout, ArchetypeId, TypeRegistry with parking_lot RwLock, register(), get(), contains(), ids(), archetype_for(), assign_to_archetype()

### Task T-CORE-2.2: Archetype and SoA Columns

- [x] **T-CORE-2.2** Implement Archetype (id: u32, component_ids: Vec<u32>, columns: Vec<Vec<u8>>, entities: Vec<EntityId>, row_count: usize); column access by component ID; entity lookup by row; row removal with swap_remove (preserve density)
  ...
  - **Effort:** 2 days
  - **VERIFIED 2026-06-03:** crates/renderer-backend/src/component_store.rs implements Archetype struct with id (ArchetypeId), component_ids, columns (Vec<Vec<u8>>), entities, free_rows for row reuse; alive_count() method

### Task T-CORE-2.3: ComponentStore (archetype registry + entity operations)

- [x] **T-CORE-2.3** Implement ComponentStore with spawn(entity_id, component_values), despawn(entity_id), read_field(entity_id, component_id, field_offset), write_field(entity_id, component_id, field_offset, value), query(component_ids) returning entity row references, column_slice(component_id) returning &[u8], all behind Arc<RwLock<ComponentStore>>
  ...
  - **Effort:** 2.5 days
  - **VERIFIED 2026-06-03:** crates/renderer-backend/src/component_store.rs (659 lines) implements ComponentStore with spawn(), despawn(), read_field(), write_field(), query() (superset match), column_slice(); global singleton via initialize_component_store() + global_component_store(); 25+ comprehensive tests

### Task T-CORE-2.4: CommandBuffer and Structural Changes

- [x] **T-CORE-2.4** Implement CommandBuffer: spawn_command, despawn_command, add_component_command, remove_component_command; flush() applies all mutations to ComponentStore atomically; replay() for determinism verification; clear() for frame boundary
  ...
  - **Effort:** 1.5 days
  - **VERIFIED 2026-06-03:** crates/renderer-backend/src/command_buffer.rs (282 lines) implements Command enum (Spawn, Despawn, AddComponent, RemoveComponent), CommandBuffer with push(), spawn_command(), despawn_command(), add_component_command(), remove_component_command(), flush(), replay(), clear(), pending(), last_flushed(); archetype migration implemented

### Task T-CORE-2.5: HierarchicalChecksum and System Phases

- [x] **T-CORE-2.5a** Implement HierarchicalChecksum: per-entity rolling checksum of component data; world-level checksum from entity checksums; frame boundary verification endpoint; xxhash-like fast hash
  ...
  - **Effort:** 1 day
  - **VERIFIED 2026-06-03:** crates/renderer-backend/src/checksum.rs (427 lines) implements HierarchicalChecksum with xxhash64-like fast_hash(), per-entity/per-component rolling checksums, atomic world_checksum(), update(), reset(); 17 comprehensive tests for determinism, accumulation, thread safety

- [x] **T-CORE-2.5b** Implement SystemPhase (ordered collection of systems) and SystemContext (delta_time: Fixed32, command_buffer: &mut CommandBuffer, world_checksum: &HierarchicalChecksum); system trait with run(context) method
  ...
  - **Effort:** 1 day
  - **VERIFIED 2026-06-03:** crates/renderer-backend/src/system_phase.rs (329 lines) implements SystemContext (delta_time, frame_number), System trait with run(), SystemPhase (name, enabled, systems, run_all()), PhaseGraph (DAG with topological_order(), execute()); closure impl for System

### Task T-CORE-2.6: ECS Tests

- [x] **T-CORE-2.6** Write tests: archetype move (add/remove component changes archetype), 10k entity spawn/despawn stress, concurrent query from 4 threads, command buffer flush correctness, checksum determinism across identical workloads, system phase execution ordering, column_slice GPU readiness
  ...
  - **Effort:** 1 day
  - **VERIFIED 2026-06-03:** component_store.rs has 25+ tests covering spawn/despawn, read/write, query, column_slice, archetype deduplication, row reuse; checksum.rs has 17 tests for determinism and accumulation; system_phase.rs integrated tests in scheduler.rs
...
- [x] **T-CORE-3.1** Implement ThreadPool: spawn N worker threads; each thread has local deque; stolen tasks from sibling deques; DEFAULT_WORKER_COUNT = 0 means auto-detect; shutdown joins all threads gracefully; priority-aware scheduling (6 levels)
  ...
  - **Effort:** 1.5 days
  - **VERIFIED 2026-06-03:** crates/renderer-backend/src/thread_pool.rs (227 lines) implements ThreadPool with Priority enum (6 levels: Critical, High, AboveNormal, Normal, BelowNormal, Background), crossbeam Injector per priority, work-stealing worker loop, spawn(priority, task), shutdown(), new_auto() for CPU auto-detection

### Task T-CORE-3.2: JobGraph and Dependencies

- [x] **T-CORE-3.2** Implement JobGraph (compiled dependency DAG), JobGraphBuilder (add_task, depends_on, finalize), TaskHandle (non-blocking is_complete), execution engine (submit job graph, workers execute tasks that have all dependencies met)
  ...
  - **Effort:** 1.5 days
  - **VERIFIED 2026-06-03:** crates/renderer-backend/src/job_graph.rs (501 lines) implements JobGraphBuilder (add_task, depends_on, finalize), JobGraph (compiled DAG), TaskHandle (is_complete, wait), CycleError, Kahn's topological sort, execute() with completion channel; 8 tests: linear_chain, diamond_dependency, cycle_detected, unknown_dependency, task_handle, empty_graph, fan_out_fan_in

### Task T-CORE-3.3: parallel_for

- [x] **T-CORE-3.3** Implement parallel_for(range, chunk_size, function) splitting range into chunks distributed via job graph; configurable chunk size (adaptive: auto-sized to worker count); returns when all chunks complete
  ...
  - **Effort:** 1 day
  - **VERIFIED 2026-06-03:** crates/renderer-backend/src/thread_pool.rs includes parallel_for(pool, range, chunk_size, f) with auto-chunking (chunk_size=0 uses num_workers), completion tracking via AtomicUsize and Condvar, blocks until all chunks complete

### Task T-CORE-3.4: Task System Tests

- [x] **T-CORE-3.4** Write tests: thread pool work-stealing verification, job graph dependency correctness, cycle detection, 10k task throughput, parallel_for index coverage, priority inversion test (high priority tasks always complete before low priority), single-threaded mode (0 workers) determinism
  ...
  - **Effort:** 0.5 day
  - **VERIFIED 2026-06-03:** thread_pool.rs has 2 tests (basic_spawn, auto_detect); job_graph.rs has 8 tests; scheduler.rs has 12 tests covering single-threaded and parallel modes, dependency ordering, frame number increments, checksum propagation
