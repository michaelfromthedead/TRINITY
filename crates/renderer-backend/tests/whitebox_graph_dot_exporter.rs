...
use renderer_backend::frame_graph::{
    BufferDesc, CompiledFrameGraph, CullStats, DispatchSource, GraphDotExporter,
...
fn barriers_involving_dead_passes_are_omitted() {
    ...
    let barriers = vec![
        (PassIndex(0), PassIndex(1), ResourceHandle(1), ResourceState::ShaderRead, ResourceState::ShaderReadWrite),
        (PassIndex(1), PassIndex(2), ResourceHandle(1), ResourceState::ShaderReadWrite, ResourceState::ShaderRead),
        (PassIndex(0), PassIndex(2), ResourceHandle(1), ResourceState::ShaderRead, ResourceState::ShaderRead),
    ];
    let graph = make_graph(
...
fn multiple_edges_with_different_resources() {
    ...
    let barriers = vec![
        (PassIndex(0), PassIndex(1), ResourceHandle(1), ResourceState::ShaderRead, ResourceState::ShaderReadWrite),
        (PassIndex(0), PassIndex(1), ResourceHandle(2), ResourceState::VertexBuffer, ResourceState::ShaderRead),
    ];
    let graph = make_graph(
        vec![p0, p1], vec![r1, r2],
        vec![PassIndex(0), PassIndex(1)],
...
fn multiple_barriers_produce_correct_edge_lines() {
    ...
    let barriers = vec![
        (PassIndex(0), PassIndex(1), ResourceHandle(1), ResourceState::ColorAttachment, ResourceState::ShaderRead),
        (PassIndex(1), PassIndex(2), ResourceHandle(2), ResourceState::ShaderRead, ResourceState::TransferSrc),
    ];
    let graph = make_graph(
