...
// ---------------------------------------------------------------------------
// Convenience functions for immediate IrResource construction
// ---------------------------------------------------------------------------

/// Creates a simple 2D texture [`IrResource`] with the given handle, name
/// and dimensions (default mip/array/format/lifetime/initial-state).
...
/// Creates a simple buffer [`IrResource`] with the given handle, name and
/// size (default storage usage, transient lifetime, uninitialised state).
pub fn mock_resource_buffer(
    handle: ResourceHandle,
    name: &str,
    size: u64,
) -> IrResource {
