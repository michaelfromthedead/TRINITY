...
/// lifetime. Data-descriptor only — no wgpu objects.
///
...
impl CompiledFrameGraph {
    ...
    pub fn emit_schedule_bridge(&self) -> serde_json::Value {
        ...
        for (from, to, before, after) in &self.barriers {
...
/// Eliminates passes whose outputs are never consumed by any other pass.
///
/// A pass is "dead" when ALL of its write resources satisfy:
/// 1. No subsequent pass reads them, OR
/// 2. The only "reader" is the pass itself (self-read in a read-modify-write).
///
/// Graphics passes are never eliminated (they render to the framebuffer /
/// swapchain, which counts as an observable side effect).
///
/// # Returns
///
/// A tuple `(passes, pruned_order, eliminated_indices, cull_stats)`.
fn eliminate_dead_passes(
    passes: Vec<IrPass>,
    order: &[PassIndex],
    _edges: &[IrEdge],
    ...
    // Build a resource handle -> readers map via passes' access sets.
    let mut resource_readers: HashMap<ResourceHandle, Vec<PassIndex>> =
        HashMap::new();
    ...
    // Collect unique write resources from eliminated passes.
    let mut freed_handles: HashSet<ResourceHandle> = HashSet::new();
    for &idx in &eliminated {
        if let Some(pass) = passes.iter().find(|p| p.index == idx) {
            for &w in &pass.access_set.writes {
                freed_handles.insert(w);
...
/// A `HashSet<PassIndex>` containing every pass that is transitively live.
/// Passes NOT in the returned set can be safely eliminated.
fn compute_transitive_liveness(
    passes: &[IrPass],
    edges: &[IrEdge],
) -> HashSet<PassIndex> {
    // Build reverse adjacency: for each producer, the set of consumers
