...
//
// reduce_min.wgsl - Parallel minimum reduction shader (T-WGPU-P3.10.1).
//
// Performs parallel minimum reduction using tree reduction in workgroup shared memory.
// Supports f32 arrays with sequential addressing to avoid shared memory bank conflicts.
//
// Algorithm:
//   1. Each thread loads 2 elements from global memory, takes min, stores to shared
//   2. Tree reduction in shared memory with workgroupBarrier() between steps
//   3. Thread 0 writes final partial minimum to output
//
// For arrays > WORKGROUP_SIZE * 2, multiple passes are required:
//   - Pass 1: Reduce input to N / (WORKGROUP_SIZE * 2) partial mins
//   - Pass 2+: Reduce partial mins until single value remains
//
// Identity element: f32::MAX (3.4028235e+38)
//
// Workgroup size: 256 threads (configurable via override constant)
// Each workgroup reduces 512 elements to 1 partial minimum.
...
/// Parallel minimum reduction kernel.
///
/// Each thread loads 2 elements, performs local min, then participates
/// in tree reduction within the workgroup. Thread 0 writes the final minimum
...
fn reduce_min(
    ...
    // Load and min two elements (with bounds checking)
    // Initialize with identity element (F32_MAX)
    var local_min: f32 = F32_MAX;
    if (idx0 < params.input_size) {
        local_min = input[idx0];
    }
    if (idx1 < params.input_size) {
        local_min = min(local_min, input[idx1]);
    }

    // Store to shared memory
    shared_data[local_idx] = local_min;
    workgroupBarrier();
    ...
    // Thread 0 writes the final partial minimum for this workgroup
    if (local_idx == 0u) {
